...
//         LocalV2ray, SshV2ray, V2rayService,
//     },
// };
...
// pub type NodeStat = Arc<(Node, TcpPingStatistic)>;

// type NodeStats = Vec<NodeStat>;
// pub struct V2rayTask {
//     node_stats: Arc<Mutex<NodeStats>>,
//     prop: V2rayTaskProperty,
//     cur_node_idx: Arc<Mutex<usize>>,
//     ping_filter: FilterManager<(Node, TcpPingStatistic)>,
//     local_v2: LocalV2ray,
//     ssh_v2: SshV2ray,
// }
...
//         Self {
//             ping_filter: FilterManager::with_ping(prop.tcp_ping.filter.clone()),
//             node_stats: Arc::new(Mutex::new(vec![])),
//             local_v2: LocalV2ray::new(prop.switch.local.clone()),
//             ssh_v2: SshV2ray::new(prop.switch.ssh.clone()),
//             prop,
...
//         let retry = self.prop.subscpt.retry_failed;
...
//         let switch = self.prop.switch.clone();
//         let cur_node_idx = self.cur_node_idx.clone();
//         tokio::spawn(async move {
//             switch_task(node_stats, switch, cur_node_idx).await.unwrap();
//         });
//     }
// }
...
//     node_stats: Arc<Mutex<NodeStats>>,
//     switch: SwitchTaskProperty,
//     cur_node_idx: Arc<Mutex<usize>>,
// ) -> Result<()> {
//     let fm: FilterManager<Arc<(Node, TcpPingStatistic)>> =
//         FilterManager::with_switch(switch.filter.clone());
//     let task = {
...
//                     let ns = node_stats.iter().map(Arc::clone).collect::<Vec<_>>();
//                     let ns = fm.before_filter(ns);
//                     continue;
//                 } else {
//                     log::debug!(
...
// async fn update_tcp_ping_stats_owned<T: V2rayService + 'static>(
//     stats: Arc<Mutex<Vec<(Node, TcpPingStatistic)>>>,
//     v2: T,
//     pp: Arc<PingProperty>,
//     fm: FilterManager<(Node, TcpPingStatistic)>,
// ) -> Result<()> {
//     // 防止过度加锁 移出nodes
...
//     let node_stats = tcp_ping::tcp_ping_nodes(v2, nodes, &pp).await;
...
// async fn switch_v2ray_ssh<'a, T: V2rayService>(
//     node_stats: Arc<Mutex<Vec<&'a (Node, TcpPingStatistic)>>>,
//     v2: T,
//     fm: FilterManager<&'a (Node, TcpPingStatistic)>,
// ) -> Result<()> {
//     let node_stats = node_stats.lock().await;
//     let ns = fm.before_filter(node_stats.to_vec());
//     ns.iter().map(|ns| ns.0.clone()).collect::<Vec<_>>();
...
// //         let tcp_ping = self.prop.tcp_ping.clone();
// //         let cur_node_idx = self.cur_node_idx.clone();
...
// //         let path = &self.prop.subscpt.path;
